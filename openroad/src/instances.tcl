...
for {set i 0} {$i < 16} {incr i} {
    set wl_act_sram($i) [format {%s.banks_gen\[%d\].i_sram.%s} $HWPE $i $SRAM_128x32]
}
# memory banks
set sram {\[0\].i_sram.}
...
set WL_INSTR  "$USER.i_wakelet_user.i_instr_mem.i_sram.gen_128x32xBx1.i_cut"
set WL_DATA   "$USER.i_wakelet_user.i_data_mem.i_sram.gen_128x32xBx1.i_cut"
# JTAG request and response CDCs
# Note: --keep-hierarchy disabled due to modport issues in vendored libs
# JTAG CDC timing constraints may not be fully applied - acceptable for validation at 80 MHz
